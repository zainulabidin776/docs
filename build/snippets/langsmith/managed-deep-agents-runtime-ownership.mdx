...
| Durable memory | Managed runtime when enabled | Optional through a project-root `memory.py` declaration and shared across the deployment. See [Memory](/langsmith/python/managed-deep-agents-memory). |
...
For author-configured fields, see [Agent definition](/langsmith/python/managed-deep-agents-agent-definition).
